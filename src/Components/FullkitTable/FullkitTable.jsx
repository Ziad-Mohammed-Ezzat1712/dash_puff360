...
export default function FullkitTable({ products = [], onEdit, onDelete }) {
  const [expandedProducts, setExpandedProducts] = useState({});

  // فلترة منتجات Full Kit فقط
  const fullkitProducts = products.filter(
    (product) =>
      Array.isArray(product.device) &&
      product.device.length >= 0 &&
      product.data?.sub_category_en === "full_kit"
  );
  ...
  const {
    ...
    currentProducts,
    totalPages,
  } = useTableSearch(fullkitProducts, "device", 3);
  ...
  if (fullkitProducts.length === 0) {
    return (
      <div className="bg-white p-6 rounded-lg shadow text-center text-gray-500">
        لا يوجد منتجات Full Kit حالياً
      </div>
    );
  }
