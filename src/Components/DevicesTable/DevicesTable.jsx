...
export default function DevicesTable({ products = [], onEdit, onDelete }) {
  ...
  const deviceProducts = products.filter(
    (product) => Array.isArray(product.device) && product.device.length >= 0
  );
   ...
   const {
      ...
      currentProducts,
      totalPages,
    } = useTableSearch(deviceProducts, "device", 3);
