...
if (editItem.category_key === "device" && editItem.device.length > 0) {
  const device = editItem.device[0];

  // COLOR KEYS (زي Postman بالظبط)
  formData.append("color_en", device.color_en);
  formData.append("color_ar", device.color_ar);

  // NEW COLOR IMAGES
  if (deviceImageFiles[0]) {
    deviceImageFiles[0].forEach((file) => {
  ...
  device.images.forEach((img) => {
    formData.append("old_color_images[]", img);
  });
}
...
>
  ...
  <div className="grid grid-cols-2 gap-4">
    <select
      ...
      onChange={(e) => {
        const brand = brands.find((b) => b.name_en === e.target.value);
    ...
    <select
      ...
      onChange={(e) => {
        const brand = brands.find((b) => b.name_ar === e.target.value);
        setEditItem({ ...editItem, brand });
      }}
      className="border p-2"
    >
  ...
  {editItem.category_key === "liquid" && (
    <div className="grid grid-cols-2 gap-4">
      <select
        ...
        onChange={(e) => {
          const type = types.find((t) => t.name_en === e.target.value);
      ...
      <select
        ...
        onChange={(e) => {
          const type = types.find((t) => t.name_ar === e.target.value);
          setEditItem({ ...editItem, type });
        }}
        className="border p-2"
      >
  ...
  {editItem.category_key === "liquid" && editItem.liquid && (
    <div className="border p-4 rounded space-y-2">
        ...
        <div key={f} className="grid grid-cols-2 gap-2">
          <input
            ...
            onChange={(e) =>
              setEditItem({
                ...editItem,
                liquid: { ...editItem.liquid, [`${f}_en`]: e.target.value },
              })
            }
            className="border p-2"
            placeholder={`${f} EN`}
          ...
          <input
            ...
            onChange={(e) =>
              setEditItem({
                ...editItem,
                liquid: { ...editItem.liquid, [`${f}_ar`]: e.target.value },
              })
            }
            className="border p-2"
            placeholder={`${f} AR`}
      ...
      <input
        type="file"
        multiple
        onChange={(e) => setLiquidImageFiles(Array.from(e.target.files))}
        className="border p-2 w-full"
      />
