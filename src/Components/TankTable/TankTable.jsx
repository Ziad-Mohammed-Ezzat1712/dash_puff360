...
// export default function TankTable({ products = [], onEdit, onDelete }) {

//   // فلترة أي منتج عنده liquid فعلاً
//   const deviceProducts = products.filter(
//     (product) => Array.isArray(product.device) && product.device.length >= 0 && product.data?.sub_category_en === "Tank"
//   );
...
export default function TankTable({ products = [], onEdit, onDelete }) {
  ...
  // فلترة المنتجات (Tank فقط)
  const deviceProducts = products.filter(
    (product) =>
      Array.isArray(product.device) &&
      product.device.length >= 0 &&
      product.data?.sub_category_en === "Tank"
  );
