...
export default function EditDevices({ editItem, onClose, refreshProducts }) {
  ...
  useEffect(() => {
    ...
    setProductData({
      ...
      category_en: editItem.data?.category_en || "device",
      category_ar: editItem.data?.category_ar || "جهاز",
      brand_en: editItem.data?.brand_en || "",
      brand_ar: editItem.data?.brand_ar || "",
      sub_category_en: editItem.data?.sub_category_en || "",
      sub_category_ar: editItem.data?.sub_category_ar || "",
      image: null,
      previewImage: editItem.data?.image || null,
    });

    setVariants(
      editItem.device?.length
        ? editItem.device.map((v) => ({
...
const removeVariant = (index) => {
  ...
  return (
    <div className="max-h-[90vh] overflow-y-auto p-6">
      ...
      >
        <h2 className="text-3xl font-bold border-b pb-4">Edit Device Product</h2>
